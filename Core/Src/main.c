...
enum State{Heating=0,Ready=1}State;
...
void PID(void)
{
	...
	if (PWM > 500)		PWM = 500;
	if (PWM < 0)		PWM = 0;
	if(Current_Temperature >= 495) PWM = 0;

	LL_TIM_OC_SetCompareCH1		(TIM2, PWM);

	SetDsiplayCurrentPWM(PWM);

	/*if(((Current_Temperature - Target_Temperature) < DeltaTempDisplay) && ((Current_Temperature - Target_Temperature) > -DeltaTempDisplay))
	{//Ready
		if(State!=Ready)
		{
  	  ...
  	  if(UpgradeDisplay == 1)
	  {
  	  ...
  	  if(PID_Enable==1)
  	  {
  		  ...
  		  PID_Enable=0;
  	  	  ...
  	  	  //uhADCxConvertedData_Voltage_mVolt = __LL_ADC_CALC_DATA_TO_VOLTAGE(VDDA_APPLI, aADCxConvertedData[2], LL_ADC_RESOLUTION_12B);
  	  	  //Temperature = ((1.45 - uhADCxConvertedData_Voltage_mVolt) / 4.3) + 25;
    }
    /* USER CODE END 3 */
  }
