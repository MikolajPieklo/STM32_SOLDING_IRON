...
void  TIM2_PWM_Init(void)
{
  ...
  LL_TIM_SetPrescaler			(TIM2, 599);  	/* Set the pre-scaler value to have TIM2 counter clock equal to 10 kHz */
  LL_TIM_EnableARRPreload		(TIM2);
  LL_TIM_SetAutoReload			(TIM2, 500);	/* Set the auto-reload value to have a counter frequency of 100 Hz */

  /*********************************/
  /* Output waveform configuration */
  /*********************************/
  LL_TIM_OC_SetMode				(TIM2, LL_TIM_CHANNEL_CH1, LL_TIM_OCMODE_PWM1);  		/* Set output mode */
  LL_TIM_OC_SetPolarity			(TIM2, LL_TIM_CHANNEL_CH1, LL_TIM_OCPOLARITY_HIGH);  	/* Set output channel polarity */
